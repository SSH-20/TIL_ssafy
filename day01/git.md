...
> Branch의 장점
1. 독립된 개바 환경을 형성하기 때문에 원본에 대해 안전
2. 하나의 작업은 하나의 브랜치로 나누어 진행됨. 체계적으로 개발 가능
3. 손쉽게 브랜치 생성, 브랜치 사이를 이동 가능

> Branch 사용 이유
1. 브랜치를 통해 별도의 작업 공간 생성
2. 브랜치에서 에러 발생한 버전을 이전으로 되돌리거나 삭제
3. 브랜치는 완전하게 독립되어있어 작업 내용이 master 브랜치에 아무런 영향을 못 끼침
4. 이후 에러가 해결됐다면? 그 내용을 master 브랜치에 반영 가능

> 명령어
- git branch : 현재 브랜치 목록을 볼 수 있다
- git branch -r : 연결된 원격 저장소를 볼 수 있다
- git branch (name) : name의 브랜치를 생성
- git switch (feature) : 브랜치를 이동할 수 있는 명령어 (원래는 HEAD가 기능을 수행한다, 이전에는 checkout을 사용했음)

* switch를 쓰기 전에 반드시 add commit을 진행해야 한다.  
-> 그 이유는 A 브랜치에서 작성한 파일은 A 에서 add commit해야 반영되기 때문이다.
* 브랜치는 작업 영역을 구분하는 것이다.
* 브랜치가 생성된 시점의 master의 log(HEAD)를 갖고 생성된다.

## Merge
> fast-forward
![스크린샷](../fast-forward.png)
- fast-forward 방식은 master가 뒤 버전에 있고 브랜치가 앞 브랜치에 있을 때, master가 브랜치의 위치를 가리키게 된다.

> 3-way-merge
![스크린샷](../3-way-merge.png)
- master와 브랜치의 분기가 서로 갈라졌을 때(브랜치가 갈라져 작업하고, 마스터가 추가 작업이 진행되었을 때)
- master가 hotfix의 commit을 다 반영하여 합쳐진 새로운 버전을 생성한다.
- master의 입장에서는 master와 hotfix의 버전을 모두 볼 수 있지만 hotfix는 마스터의 버전을 볼 수 없다.
- 이후 hotfix가 merge를 하게 되면 그때는 fast-forward로 진행된다.  
-> hotfix는 master의 작업을 열람할 필요가 없기 때문

* fork와 비슷해 보일 수 있으나 fork는 똑같지만 별개의 프로젝트를 내려받는 것. branch는 같은 프로젝트를 별개의 작업 영역에서 작업하는 것.

> Merge-conflict
- 3-way와 비슷한 상황에서 merge를 할 때, master에서 수정한 내용과 hotfix가 수정한 내용이 겹칠 때 git이 

## 팀 프로젝트의 git 관리 (3인)
- master는 relese할 수 있을 때 Merge를 진행한다.
- 팀의 각 개인이 branch를 만들어 프로젝트 작업을 진행한다.
- 각자가 작업된 작업물을 dev(중간저장소) 브런치에서 merge하여 작업물을 올린다
- 만약 본인의 영역 외의 코드를 수정하거나 conplict가 발생하면 팀원에게 꼭 알린다.(수정할 때도 마찬가지)
- AI를 사용하다보면 의도치 않은 구역의 코드 수정이 일어나 conplict가 자주 발생할 수 있으니 반드시 팀원과 상의를 진행하자
